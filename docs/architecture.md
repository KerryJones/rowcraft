...
- **No segment type field** — segments have no `type` field. Behavior is derived from content: rest = no `target_intensity` AND no `target_stroke_rate`. `target_hr_zone` is a derived field auto-computed from `target_intensity` at build/save time using the 5-zone model (<55%→null, 55-75%→Z1, 75-85%→Z2, 85-92%→Z3, 92-97%→Z4, ≥97%→Z5). Display code reads the stored zone; rest segments (no intensity or below 55%) show gray.
...
- **Post-workout flow** — Structured segments complete → `structuredComplete` phase → completion modal (Continue/Save/Discard). "Continue" appends a free-row segment and resumes rowing. "Save" → summary screen (stats, pace/HR charts, splits) → save/discard. Entire session (structured + free cool-down) is saved as one `WorkoutResult`.
...
- **BLE device memory** — on first connect, `BleProvider` emits `pendingRememberDevice` instead of auto-saving. The connect screen shows a "Remember this device?" dialog. Only confirmed devices are persisted to `SavedDevices` (Drift). `confirmRememberDevice()` / `dismissRememberDevice()` clear the pending state.
- **Screen always-on** — `WakelockPlus.enable()` is called in `WorkoutScreen.initState()` and released in `dispose()` so the screen never sleeps during an active workout.
- **Countdown audio** — `AudioService` (singleton) generates PCM WAV beeps in-memory (no asset files). Short 880 Hz beep plays at T-3/2/1s; higher 1320 Hz beep at T-0 (segment start). `workout_screen.dart` fires beeps via PM5 data events when segment remaining ≤ 3s for time-based segments.
